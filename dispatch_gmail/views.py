...
def get_gmail_incidents(payload):
    '''
        ...
        for response_part in msg_data:
            if isinstance(response_part,tuple):
                ...
                try:
                  ...
                  sender = msg["From"]
                  subject = msg["Subject"]
                  email_fields_list = [msg, payload, body, datetime_str, sender, subject]
                  email = Email.objects.create(**email_fields_list)
                  email.save()
                except IndexError:
...
def extract_gmail_incidents(request):
    total = Email.objects.all().count()
    print ("Total Email Incidents:", total)
    email_incident_list = Email.objects.all()
    ...
    for email in email_incident_list:
        try:

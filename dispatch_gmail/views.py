        ...
        for response_part in msg_data:
            if isinstance(response_part,tuple):
                ...
                try:
                  ...
                  # Convert email received time to a DateTime object.
                  sent = msg['Date']
                  #pdb.set_trace()
                  if bool(re.search("-0700", sent)) == True:
                    date = datetime.strptime(sent, '%d %b %Y %H:%M:%S -0700' )
                  else:
                    date = datetime.strptime(sent, '%d %b %Y %H:%M:%S -0800' )

                  # Save the Incident to the database.
                  incident = Incident.objects.create(recieved = date, **incident_dict)
                  incident.save()
